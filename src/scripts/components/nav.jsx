...
const Navegador = ({ expand, show, handleClose }) => {
  ...
  const [mostrarParticipantesModal, setMostrarParticipantesModal] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const notificacionesRef = ref(
      database,
      `notificaciones/usuarios/${ini.uid}`
    );
    ...
    if (esAdmin) {
      ...
      notificacionesAdminUnsubscribe = onValue(
        notificacionesAdminRef,
        (snapshot) => {
          if (snapshot.exists()) {
            ...
            setNotificaciones((prevNotificaciones) => {
              ...
              const notificacionesUsuario = prevNotificaciones.filter(
                (n) => !n.esNotificacionAdmin
              );
              // Combinar y ordenar por fecha
              return [...notificacionesUsuario, ...notificacionesList].sort(
                (a, b) => {
                  if (!a.fechaCreacion) return 1;
                  if (!b.fechaCreacion) return -1;
                  return new Date(b.fechaCreacion) - new Date(a.fechaCreacion);
            ...
            setNotificacionesNoLeidas((prevCount) => {
              const adminNoLeidas = notificacionesList.filter(
                (notif) => !notif.leida
              ).length;
              // Obtener solo las no leídas de usuario del estado actual
              const usuarioNoLeidas = notificaciones.filter(
                (n) => !n.esNotificacionAdmin && !n.leida
              ).length;
    ...
    const notificacionesUsuarioUnsubscribe = onValue(
      notificacionesRef,
      (snapshot) => {
        if (snapshot.exists()) {
          const notificacionesData = snapshot.val();
          ...
          if (esAdmin) {
            setNotificaciones((prevNotificaciones) => {
              ...
              const notificacionesAdmin = prevNotificaciones.filter(
                (n) => n.esNotificacionAdmin
              );
              // Combinar y ordenar por fecha
              return [...notificacionesAdmin, ...notificacionesList].sort(
                (a, b) => {
                  if (!a.fechaCreacion) return 1;
                  if (!b.fechaCreacion) return -1;
                  return new Date(b.fechaCreacion) - new Date(a.fechaCreacion);
            ...
            // Actualizar contador de no leídas
            setNotificacionesNoLeidas((prevCount) => {
              const usuarioNoLeidas = notificacionesList.filter(
                (notif) => !notif.leida
              ).length;
              // Obtener solo las no leídas de admin del estado actual
              const adminNoLeidas = notificaciones.filter(
                (n) => n.esNotificacionAdmin && !n.leida
              ).length;
              return usuarioNoLeidas + adminNoLeidas;
            });
            ...
            const noLeidas = notificacionesList.filter(
              (notif) => !notif.leida
            ).length;
          ...
          setNotificaciones((prevNotificaciones) =>
            prevNotificaciones.filter((n) => n.esNotificacionAdmin)
          );
          setNotificacionesNoLeidas(
            notificaciones.filter((n) => n.esNotificacionAdmin && !n.leida)
              .length
          );
        }
      }
    ...
    notificaciones.forEach((notif) => {
      if (!notif.leida) {
        if (notif.esNotificacionAdmin && esAdmin) {
          ...
          updates[
            `notificaciones/usuarios/${ini.uid}/${notif.id}/leida`
          ] = true;
        }
      }
    });
    ...
    if (
      ...
      try {
        ...
        const solicitudRef = ref(
          database,
          `eventos/${notif.eventoId}/solicitudes/${notif.usuarioId}`
        );
    ...
    // Si es una notificación de participantes
      ...
      try {
        ...
        const participantesRef = ref(
          database,
          `eventos/${notif.eventoId}/participantes`
        );
  ...
  const aprobarSolicitud = async (solicitud) => {
    try {
      ...
      const solicitudRef = ref(
        database,
        `eventos/${solicitud.eventoId}/solicitudes/${solicitud.id}`
      );
      ...
      const participanteRef = ref(
        database,
        `eventos/${solicitud.eventoId}/participantes/${solicitud.id}`
      );
      ...
      const notificacionUsuarioRef = ref(
        database,
        `notificaciones/usuarios/${solicitud.usuarioId}`
      );
